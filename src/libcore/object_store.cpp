...
namespace minty::core {
    ...
    auto object_store::init(std::string_view bucket_name) -> ext::task<> {
    ...
    bucket::bucket(
        ...
        connection(std::forward<fstore::client::pool::item>(connection)),
        id(id) {}

    auto bucket::get(const UUID::uuid& object_id) -> ext::task<fstore::blob> {
        co_return co_await connection->get_object(id, object_id);
    }

    auto bucket::get(const UUID::uuid& object_id, std::byte* buffer)
        -> ext::task<> {
        co_await connection->get_object(id, object_id, buffer);
    }

    auto bucket::meta(const UUID::uuid& object_id)
        -> ext::task<fstore::object> {
        co_return co_await connection->get_object_metadata(id, object_id);
    }

    auto bucket::remove(const UUID::uuid& object_id)
        -> ext::task<fstore::object> {
        co_return co_await connection->remove_object(id, object_id);
    }

    auto bucket::remove(std::span<const UUID::uuid> objects)
