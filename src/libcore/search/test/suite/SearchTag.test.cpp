...
auto SearchTagTest::add_tag(std::string_view name)
    -> ext::task<std::reference_wrapper<const tag>> {
    const auto result =
        std::find_if(tags.begin(), tags.end(), [name](const auto& tag) {
            return std::find(tag.names.begin(), tag.names.end(), name) !=
                   tag.names.end();
        });
    ...
    const auto errors =
        co_await search.add_tags(std::span<const tag>(result, result + 1));
