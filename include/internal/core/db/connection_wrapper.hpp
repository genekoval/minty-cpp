...
namespace minty::core::db {
    ...
    public:
        ...
        auto begin() -> ext::task<pg::transaction> { return inner->begin(); }

        auto create_comment(const UUID::uuid& post_id, std::string_view content)
            -> ext::task<comment> {
        ...
        auto create_post(const UUID::uuid& post_id) -> ext::task<time_point> {
        ...
        auto create_reply(const UUID::uuid& parent_id, std::string_view content)
            -> ext::task<comment> {
        ...
        auto create_source(std::int64_t site_id, std::string_view resource)
            -> ext::task<source> {
            return inner->create_source(site_id, resource);
        }

        auto create_tag(std::string_view name) -> ext::task<UUID::uuid> {
            return inner->create_tag(name);
        }

        auto create_tag_alias(const UUID::uuid& tag_id, std::string_view alias)
            -> ext::task<tag_name> {
            return inner->create_tag_alias(tag_id, alias);
        }

        auto create_tag_source(const UUID::uuid& tag_id, std::int64_t source_id)
            -> ext::task<> {
        ...
        auto delete_post(const UUID::uuid& post_id) -> ext::task<> {
        ...
        auto update_tag_name(const UUID::uuid& tag_id, std::string_view name)
